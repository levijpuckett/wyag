...
cd left
$wyag checkout HEAD ../temp1 2> /dev/null
mkdir ../temp2
cd  ../temp2
git --git-dir=../right/.git checkout . 2> /dev/null
cd ..
diff -r temp1 temp2
rm -rf temp1 temp2
...
cd ../right
git tag -a TEST_ANNOTATED -m 'A tag generated by wyag - not customizable :)'
cd ..
# confirms that the list of tags is the same.
run_test tag 

step "annotated tag object structure"
cd left
git cat-file tag TEST_ANNOTATED | grep -v "tagger" > ../file1
cd ../right
wyag cat-file tag TEST_ANNOTATED | grep -v "tagger" > ../file2
cd ..
cmp file1 file2
...
cd left && git co main~ 2> /dev/null
cd ../right && git co main~ 2> /dev/null
cd ..
run_test branch
